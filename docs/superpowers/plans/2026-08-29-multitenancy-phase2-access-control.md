...
| D10 | **`PATCH` gains `is_primary` (promote only), behind a required `acknowledge_garmin_reassignment` flag.** | §3.1's archive guard must 409 with "promote another person first", and without a promote endpoint that message names a fix the API cannot perform. Implemented as a transactional swap inside `BEGIN IMMEDIATE` — `idx_persons_primary` is a partial `UNIQUE` index, so the old primary must be cleared before the new one is set. Demotion (`is_primary: false`) is a 422: there must always be exactly one primary, because `get_primary_person_id()` raises when there is none. **The acknowledgement is the load-bearing part.** `garmin_credential_person_id()` *is* `get_primary_person_id()`, so this is the first request-path way to move a pointer an existing security guard reads. Promoting therefore (a) repoints `scheduled_sync`, so the original human's Garmin data is written under the new primary, over any measurement they already had for those dates; (b) makes the new primary the only person `POST /p/{slug}/api/sync` accepts; and (c) 409s the previous primary out of syncing their own data. A client-side `confirm()` cannot enforce that against a scripted `PATCH`, so the acknowledgement lives in the API and the refusal names all three consequences. Phase 3's `garmin_links` removes the coupling and this flag with it. |
| D14 | **`own`-holder grant management is API-only in Phase 2.** | Spec f.6 makes grant management "admin **or** `own` on that person" and the API honours it, but `/auth/admin/persons` is `require_admin`-gated and its user dropdown reads the admin-only `/auth/admin/users/list`. Building a second, grant-scoped page is Phase 5's UI work; recorded here so the gap is a decision rather than an oversight. |
| D15 | **The `users.id` existence oracle on grant writes is accepted, not closed.** | A non-admin `own` holder can tell an existing `users.id` from a missing one by 200-vs-404 on `PUT .../grants/{user_id}`. The distinguisher is the status code, so suppressing the message would obscure rather than close it. Accepted because the secret this design protects is *person* existence, not account-id existence; `revoke_token` already gives every authenticated user the same oracle over `api_tokens.id`; each probe writes a real, self-revocable grant on a person the prober already owns; and there is no UI path to it. Revisit if grants gain a non-admin UI — addressing the target by username would make the oracle a guess rather than a count. |

<!-- Generated: 2026-08-22 | Files scanned: 22 | Token estimate: ~850 -->
# Backend
...
2. `@app.middleware("http") auth_middleware` — skips `/auth/*`, `/health`, `/static/*`;
   ...
   otherwise validates the `vf_session` HMAC cookie, 401 on `/api/*`, redirect elsewhere.
...
POST /api/weight                -> unit convert -> garmin_client.push_weight()
                                    -> INSERT weight_log
...
retried on first `/api/weight` POST).
